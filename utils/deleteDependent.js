...
const deleteContact = async (filter) =>{
  try {
    let response  = await dbService.deleteMany(Contact,filter);
    return response;
  } catch (error){
...
const deleteUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const contactFilter = { $or: [{ userId : { $in : user } },{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const contactCnt = await dbService.deleteMany(Contact,contactFilter);
...
const softDeleteContact = async (filter,updateBody) =>{  
  try {
    const contactCnt =  await dbService.updateMany(Contact,filter);
    return { contact : contactCnt };
  } catch (error){
    throw new Error(error.message);
  }
...
const softDeleteUser = async (filter,updateBody) =>{  
  try {
    ...
    if (user.length){
      ...
      const contactFilter = { '$or': [{ userId : { '$in' : user } },{ addedBy : { '$in' : user } },{ updatedBy : { '$in' : user } }] };
      const contactCnt = await dbService.updateMany(Contact,contactFilter,updateBody);
